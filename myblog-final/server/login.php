...
Hongxiu Guo: design the HTML and CSS, set session and cookie for the user
        ...
        if(mysqli_num_rows($result) > 0){//if there is a user with the same username
            ...
            if(password_verify($password, $user['passwd'])){
                ...
                if($_POST['remember'] == 'on'){
                    ...
                    setcookie('password', $_POST['password'], time() + 86400);//cookie will last for 1 day
                }
                //start a session
                session_start();
        ...
        <aside>
            ...
            <form action="login.php" method="POST" onsubmit="return validate();">
                <div class="textfield">
                    ...
                    value="<?php if(isset($_POST['username'])) echo $_POST['username']; else if(isset($_COOKIE['name'])) echo $_COOKIE['name']; ?>">
                    <span class="errorMsg" id="username_span"><?php echo $errors['username']?></span>
                </div>
                <div class="textfield">
                    <label for="password">password:</label>
                    <input type="password" name="password" id="password" 
                    value="<?php if(isset($_POST['password'])) echo $_POST['password']; else if(isset($_COOKIE['password'])) echo $_COOKIE['password'];?>">
